...
function LazyWeirdo:CreateConfig()
  ...
  ---------------------------------------
  -- Right-side list panel (New screen)--
  ---------------------------------------
  local listPanel = CreateFrame("Frame", "LazyWeirdoListPanel", UIParent)
  listPanel:SetWidth(320)
  listPanel:SetHeight(480)
  listPanel:SetPoint("TOPLEFT", UIParent, "CENTER", 0, 0)
  ...
  listPanel:SetMovable(true)
  listPanel:EnableMouse(true)
  listPanel:RegisterForDrag("LeftButton")
  listPanel:SetScript("OnDragStart", function()
    this:StartMoving()
  end)

  listPanel:SetScript("OnDragStop", function()
    this:StopMovingOrSizing()
  end)

  table.insert(UISpecialFrames, "LazyWeirdoListPanel")
  ...
  local listAddBtn = CreateFrame("Button", nil, listPanel, "UIPanelButtonTemplate")
  listAddBtn:SetHeight(18)
  listAddBtn:SetWidth(60)
  listAddBtn:SetPoint("TOPLEFT", listPanel, "TOPLEFT", 10, -26)
  listAddBtn:SetText("Add")

  local listSearch = CreateFrame("EditBox", nil, listPanel, "InputBoxTemplate")
  listSearch:SetHeight(18)
  listSearch:SetAutoFocus(false)
  listSearch:SetPoint("LEFT", listAddBtn, "RIGHT", 8, 0)
  listSearch:SetPoint("RIGHT", listPanel, "TOPRIGHT", -120, -26)
  listSearch:SetText("")
  listSearch:SetScript("OnEscapePressed", function()
    this:SetText("")
    this:ClearFocus()
  end)
  ...
  LazyWeirdoConfigFrame:SetScript("OnHide", function()

    -- keep listPanel independent; do not force-hide
  end)

  local listEnable = CreateFrame("CheckButton", nil, listPanel, "UICheckButtonTemplate")
  listEnable:SetPoint("TOPRIGHT", -80, -26)
  local listEnableText = listEnable:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
  listEnableText:SetPoint("LEFT", listEnable, "RIGHT", 2, 0)
  listEnableText:SetText("Enabled")

  local ROW_H = 18
  local listScroll = CreateFrame("ScrollFrame", "LazyWeirdoListScroll", listPanel, "FauxScrollFrameTemplate")
  listScroll:SetPoint("TOPLEFT", 8, -50)
  listScroll:SetPoint("BOTTOMRIGHT", -44, 8)
  listPanel.scroll = listScroll
  listPanel.rowHeight = ROW_H
  listPanel.visibleRows = 8
  listPanel.rows = {}
  listPanel.popup = "ADD_ITEM_NAME"
  listPanel.total = 0
  listPanel.filter = ""

  local RemoveItemFromDropdown
  local RefreshListPanel
  
  --Scroll handler for list panels; shared by all lists

  local function SetListOffset(offset)
    local total = listPanel.total or 0
    local visible = listPanel.visibleRows or 1
    local maxOffset = total - visible
    if maxOffset < 0 then maxOffset = 0 end
    if offset < 0 then offset = 0 end
    if offset > maxOffset then offset = maxOffset end
    listPanel.scroll.offset = offset
    RefreshListPanel()
  end

  function RefreshListPanel()
    if not listPanel:IsShown() then return end
    local items = listPanel.items or {}
    local filter = string.lower(listPanel.filter or "")
    local view = {}
    ...
    local total = table.getn(view)
    listPanel.total = total
    local height = listScroll:GetHeight()
    if height and height > 0 then
      local rows = math.floor(height / ROW_H)
      if rows < 1 then rows = 1 end
      listPanel.visibleRows = rows
    end
    local offset = listPanel.scroll.offset or 0
    local maxOffset = total - (listPanel.visibleRows or 1)
    if maxOffset < 0 then maxOffset = 0 end
    if offset > maxOffset then
      offset = maxOffset
      listPanel.scroll.offset = offset
      local sb = getglobal(listScroll:GetName() .. "ScrollBar")
      if sb then sb:SetValue(offset * ROW_H) end
    end

    for i = 1, listPanel.visibleRows do
      local row = listPanel.rows[i]
      if not row then
        row = CreateFrame("Button", nil, listPanel)
        row:SetHeight(ROW_H)
        row:SetPoint("TOPLEFT", 10, -50 - (i - 1) * ROW_H)
        row:SetPoint("RIGHT", -32, 0)
        row:SetHighlightTexture("Interface\\QuestFrame\\UI-QuestTitleHighlight")
        row:RegisterForClicks("LeftButtonUp", "RightButtonUp")

        local indexText = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
        indexText:SetPoint("LEFT", 0, 0)
        indexText:SetJustifyH("LEFT")
        row.indexText = indexText

        local check = CreateFrame("CheckButton", nil, row, "UICheckButtonTemplate")
        check:SetPoint("LEFT", indexText, "RIGHT", 4, 0)
        row.check = check
        ...
        local rtex = remove:CreateTexture(nil, "ARTWORK")
        rtex:SetAllPoints(remove)
        rtex:SetTexture("Interface\\Buttons\\UI-GroupLoot-Pass-Up")
        remove.icon = rtex
        row.remove = remove

        local text = row:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
        text:SetPoint("LEFT", check, "RIGHT", 2, 0)
        text:SetPoint("RIGHT", remove, "LEFT", -4, 0)
        ...
        remove:SetScript("OnClick", function()
          ...
          if not entry then return end
          local name = type(entry) == "table" and entry.name or entry
          local pop = StaticPopup_Show("REM_ITEM_NAME", name, listPanel.label)
          if pop then
            pop.data = { items = listPanel.items, item = name, label = listPanel.label }
          end
        end)

        listPanel.rows[i] = row
      end

      local idx = i + offset
      local entry = view[idx]
      if entry then
        row.entry = entry
        row:Show()
        row.indexText:SetText(idx .. ".")
        row.indexText:Show()
        if listPanel.toggleable then
          local name = entry.name or ""
          local label = entry.count and (entry.count .. "x " .. name) or name
          row.check:Show()
          ...
          row.check:Hide()
          row.text:ClearAllPoints()
          row.text:SetPoint("LEFT", row.indexText, "RIGHT", 4, 0)
          row.text:SetPoint("RIGHT", row.remove, "LEFT", -4, 0)
    ...
    for i = listPanel.visibleRows + 1, table.getn(listPanel.rows) do
      listPanel.rows[i]:Hide()
    end

    listPanel._updatingScroll = true
    FauxScrollFrame_Update(listPanel.scroll, total, listPanel.visibleRows, ROW_H)
    listPanel._updatingScroll = nil
  end

  listScroll:SetScript("OnVerticalScroll", nil)

  local listScrollBar = getglobal(listScroll:GetName() .. "ScrollBar")
  if listScrollBar then
    listScrollBar:SetScript("OnValueChanged", function()
      if listPanel._updatingScroll then return end
      local value = this:GetValue() or 0
      local offset = math.floor((value / ROW_H) + 0.5)
  ...
  listScroll:SetScript("OnMouseWheel", function()
    ...
    local offset = (listPanel.scroll.offset or 0) - delta
    local sb = getglobal(listScroll:GetName() .. "ScrollBar")
    if sb then
      sb:SetValue(offset * ROW_H)
    else
      SetListOffset(offset)
    end
  end)
  ...
  local function ShowListPanel(opts)
    listPanel.items = opts.items
    listPanel.label = opts.label
    listPanel.toggleable = opts.toggleable
    listPanel.setting = opts.setting
    listPanel.popup = opts.popup or "ADD_ITEM_NAME"
    listTitle:SetText(opts.label or "List")
    listSearch:SetText("")
    listPanel.filter = ""

    -- Position to the right of the main config frame, but keep independent parent

    listPanel:ClearAllPoints()
    if LazyWeirdoConfigFrame:IsShown() then
      local left = LazyWeirdoConfigFrame:GetLeft() or 0
      local top = LazyWeirdoConfigFrame:GetTop() or 0
      listPanel:SetPoint("TOPLEFT", UIParent, "BOTTOMLEFT", left + LazyWeirdoConfigFrame:GetWidth() + 10, top)
    else
      listPanel:SetPoint("CENTER", UIParent, "CENTER")
    end

    if opts.setting then
      listEnable:Show()
      listEnableText:Show()
      listEnable:SetChecked(LazyWeirdoDB.settings[opts.setting] and 1 or nil)
      listEnable:SetScript("OnClick", function()
  ...
  listAddBtn:SetScript("OnClick", function()
    local add = StaticPopup_Show(listPanel.popup or "ADD_ITEM_NAME", listPanel.label or "List")
    if add then
      add.data = { items = listPanel.items, label = listPanel.label, toggleable = listPanel.toggleable }
  ...
  listSearch:SetScript("OnTextChanged", function()
    listPanel.filter = this:GetText() or ""
    SetListOffset(0)
  end)
    ...
    local function InitializeDropdown()
      ...
      if addBtn then
        ...
        addBtn:SetScript("OnEnter", function()
          ...
          GameTooltip:AddLine("Manage items in the list panel on the right.", 0.5, 0.5, 0.5, true)
          GameTooltip:Show()
        end)
...
local function AddItemToDropdown(dropdown_list, item, list_label, toggleable)
  ...
  if listPanel and listPanel.items == dropdown_list then
    RefreshListPanel()
  end
end

RemoveItemFromDropdown = function(dropdown_list, item, list_label)
  ...
  if listPanel and listPanel.items == dropdown_list then
    SetListOffset(listPanel.scroll.offset or 0)
  end
end
...
local function ParseBuyInput(text)
  ...
  if listPanel and listPanel.items == LazyWeirdoDB.buylist then  
    RefreshListPanel()
  end
end

-- Static Popup Dialog for entering item names
-- Add now can cancel with Esc
...
StaticPopupDialogs["REM_ITEM_NAME"] = {
  ...
  OnShow = function()
    ...
    this:SetScript("OnKeyDown", function()
      if arg1 == "ENTER" then
        local b1 = getglobal(this:GetName() .. "Button1")
